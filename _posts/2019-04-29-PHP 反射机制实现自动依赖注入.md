---
layout: mypost
title: PHP 反射机制实现自动依赖注入
categories: [PHP]
---

依赖注入又叫控制反转，使用过框架的人应该都不陌生。简单研究一下，直接上代码
```php
/**
*
* 工具类，使用该类来实现自动依赖注入。
...
上面的代码使用 php 的反射函数，创建了一个容器类，使用该类来实现其他类的依赖注入功能。上面的依赖注入分为两种，一种是构造函数的依赖注入，一种是方法的依赖注入。 我们使用下面三个类来做下测试。
